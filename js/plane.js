 ...
 * @return {[type]} [description]
 */
Plane.prototype.drawEachScore = function() {
    var eachScore = this.eachScore;

    // 每架敌机无生命时的分数、位置是否为空
    if (eachScore !== undefined) {
        for (var i = eachScore.length - 1; i >= 0; i--) {
            var eachScoreIndex = eachScore[i];
            eachScoreIndex.y -= 0.5;
            eachScoreIndex.opacity -= 1/50;
            ctx.font = '20px arial';
            ctx.fillStyle = 'rgba(255,255,255,' + eachScoreIndex.opacity + ')';
            ctx.fillText(eachScoreIndex.score, eachScoreIndex.x, eachScoreIndex.y);
            this.remain++;
        }
       
        // 50帧之后删除敌机分数
        if (this.remain > 50) {
            eachScore.splice(0, 1);
            this.remain = 0;
        }
    }
 ...
 * @param {[type]} target [description]
 */
Plane.prototype.drawScore = function() {
    ctx.font = '20px arial';
    ctx.fillStyle = '#fff';
